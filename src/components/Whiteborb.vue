...
export default class Whiteborb extends Vue {
  ...
  private currentLine = new fabric.Line();
  ...
  private mounted() {
    ...
    this.socket.on('freeDrawing', (drawData: any) => {
      this.currentLine.x1 = drawData.startX;
      this.currentLine.y1 = drawData.startY;
      this.currentLine.x2 = drawData.endX;
      this.currentLine.y2 = drawData.endY;
  ...
  private listenToMouseMove() {
    this.canvas.on('mouse:move', (event: any) => {
      ...
      if (this.toolSelected === Tools.Pen) {
        this.currentLine.set({ x2: mouse.x, y2: mouse.y });
        this.canvas.renderAll();
        // this.sendCanvas();

        this.socket.emit('freeDrawing', {
          roomName: this.roomName,
